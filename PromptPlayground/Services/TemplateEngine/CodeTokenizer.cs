    ...
    public List<Block> Tokenize(string? text)
    {
        ...
        if (text.Length == 1)
        {
            switch (nextChar)
            {
                case Symbols.VarPrefix:
                    blocks.Add(new VarBlock(text, _logger));
                    break;

                case Symbols.DblQuote:
                case Symbols.SglQuote:
                    blocks.Add(new ValBlock(text, _logger));
                    break;

                default:
                    blocks.Add(new FunctionIdBlock(text, _logger));
                    break;
            }
        ...
        for (int nextCharCursor = 1; nextCharCursor < text.Length; nextCharCursor++)
        {
            ...
            if (currentTokenType == TokenTypes.Value)
            {
                ...
                {
                    blocks.Add(new ValBlock(currentTokenContent.ToString(), _logger));
            ...
            if (IsBlankSpace(currentChar))
            {
                ...
                {
                    blocks.Add(new VarBlock(currentTokenContent.ToString(), _logger));
                    currentTokenContent.Clear();
                }
                else if (currentTokenType == TokenTypes.FunctionId)
                {
                    blocks.Add(new FunctionIdBlock(currentTokenContent.ToString(), _logger));
                    currentTokenContent.Clear();
                }
