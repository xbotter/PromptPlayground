...
internal sealed class TemplateTokenizer
{
    ...
    public TemplateTokenizer(ILogger? logger = null)
    {
        ...
        _codeTokenizer = new CodeTokenizer(_logger);
    }
    ...
    public IList<Block> Tokenize(string? text)
    {
        ...
        {
            return new List<Block> { new TextBlock(string.Empty, _logger) };
        }

        // If the template is "empty" return the content as a text block
        if (text.Length < MinCodeBlockLength)
        {
            return new List<Block> { new TextBlock(text, _logger) };
        }
            ...
            if (blockStartFound)
            {
                ...
                else
                {
                    ...
                    else if (currentChar == Symbols.BlockEnder && nextChar == Symbols.BlockEnder)
                    {
                        ...
                        {
                            blocks.Add(new TextBlock(text, endOfLastBlock, blockStartPos, _logger));
                        }
                        ...
                        if (contentWithoutDelimiters.Length == 0)
                        {
                            ...
                            blocks.Add(new TextBlock(contentWithDelimiters, _logger));
                        }
                        else
                        {
                            ...
                            switch (codeBlocks[0].Type)
                            {
                                ...
                                case BlockTypes.FunctionId:
                                    ...
                                    blocks.Add(new CodeBlock(codeBlocks, contentWithoutDelimiters, _logger));
                                    break;
        ...
        {
            blocks.Add(new TextBlock(text, endOfLastBlock, text.Length, _logger));
        }
