...
namespace PromptPlayground.ViewModels
{
	public partial class SemanticFunctionViewModel : ViewModelBase, IEquatable<SemanticFunctionViewModel>
	{
		...
		internal PromptTemplateConfig PromptConfig => PromptTemplateConfig.FromJson(Config);
		...
		public async Task GenerateResultAsync(CancellationToken cancellationToken)
		{
			try
			{
				...
				var allPlugins = WeakReferenceMessenger.Default.Send(new RequestMessage<List<PluginViewModel>>());
				var service = new PromptService(configProvider.Response, allPlugins.Response);
				...
				var tasks = results
					 .Select(async r =>
					 {
						 ...
						 if (GetRunStream())
						 {
							 var results = service.RunStreamAsync(this, new KernelArguments(arguments), cancellationToken);
						 ...
						 {
							 var result = await service.RunAsync(this, new KernelArguments(arguments), cancellationToken);
							 r.PromptRendered = result.PromptRendered;
							 r.Text = result.Text;
							 r.Elapsed = result.Elapsed;
							 r.Error = result.Error;
